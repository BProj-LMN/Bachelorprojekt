...
int Wrapper_Socket::updateIstwerte() {
  ...
  if (Socket->get_message(Nachricht) == 1) {      // bei neuer Nachricht
    if ((Nachricht[7] & 0x01) == 0x00) {          // wenn etwas getrackt wurde
      if (0xDA == ((char) Nachricht[0] & 0xFF)) { // wenn Nachrichtentyp = Trackingdaten
        ...
        std::cout << "keine gueltige Nachricht" << std::endl;
      }
    }
  }

  /*
   if ((istX > 3850) || (istX < 250)) {
   return 1;
   }
   if ((istY > 3750) || (istY < 250)) {
   return 1;
   }
   if (istZ > 2550) {
   return 1;
