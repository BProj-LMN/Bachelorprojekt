...
int UserInterface::sollEinlesen() {
	...
	case 0:cout << "Sollwertvorgabe" << endl;
		...
		status = 1;
		break;
	case 1:cout << "X-Koordinate eingeben (300-3800 mm)" << endl;
		status = 4;
		pos = 0;
		break;
	case 2: cout << "Y-Koordinate eingeben (300-3700 mm)" << endl;
		status = 4;
		break;
	case 3:cout << "Z-Koordinate eingeben (300-2500 mm)" << endl;
		status = 4;
		break;
		//Zahl aufnehemen
	case 4: if (_kbhit()) {// Wenn eine Taste betätigt wurde
				char eingabe = _getch();//die gedrückte taste wird eingelesen
				fflush(stdin);//Buffer leeren
	...
	case 5:for (int i = 0; i < zaehler; i++) {//läuft den gesamten buffer durch
		   ...
		   switch (pos){
		   case 0:if (soll[pos] > 3800){
					  soll[pos] = 3800; }
				  if (soll[pos]<300){
					  soll[pos] = 300;}
				  break;
		   case 1:if (soll[pos]>3700){
					  soll[pos] = 3700;}
				  if (soll[pos]<300) {
					  soll[pos] = 300; }
				  break;
		   case 2:if (soll[pos]>2500){ 
					  soll[pos] = 2500; }
				  if (soll[pos] < 300) {
					  soll[pos] = 300;}
				  break;
		   ...
		   //if(soll[pos]>4500)soll[pos]=4500;
