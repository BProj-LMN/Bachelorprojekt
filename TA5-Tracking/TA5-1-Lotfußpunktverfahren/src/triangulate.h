...
float triangulate(float *K1_vec, float *u_vec, float *K2_vec, float *v_vec) {
    ...
    float y[MAX];
    float Skalar[MAX];
    float Verbindungsvektor[MAX + 1];
    float Verbindungsvektor_halbiert[MAX + 1];
    float Schnittpunkt[MAX + 1];
    ...
    for (S0 = 0; S0 < (MAX - 1); S0++) {
        Pivotelement = x[S0][S0];
        for (S2 = 1; S2 < (MAX - S0); S2++) {
            q = ((x[(S2 + S0)][S0] / Pivotelement) * (-1));
            for (S3 = 0; S3 < (MAX - S0); S3++) {
    ...
    for (S4 = (MAX - 1); S4 >= 0; S4--) {
        Summe = 0;
        for (S5 = (S4 + 1); S5 <= (MAX - 1); S5++) {
    ...
    /*** Ausgabe der beiden Skalare ***/
    /*
    printf("\nDie beiden Skalare sind\n\n", MAX);
    for (i = 0; i < MAX; i++)
        printf("\tSkalar%d = %1.2f\n", i + 1, Skalar[i]);
    printf("\n\n");
    printf("\nDer Verbindungsvektor ist\n\n", MAX);
    for (i = 0; i <= MAX; i++)
        printf("\tX%d = %1.2f\n", i + 1, Verbindungsvektor[i]);
    printf("\n\n");
    printf("\nDer Schnittpunkt im globalen Koordinatensystem ist\n\n", MAX);
    for (i = 0; i <= MAX; i++)
        printf("\tX%d = %1.2f\n", i + 1, Schnittpunkt[i]);
    printf("\n\n");
    */
